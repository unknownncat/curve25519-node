...
- X25519 + Ed25519 + axlsign (optional native mode via Rust `napi-rs`)
...
Note: the `napi` namespace is only available when a native addon exists for the target
`${process.platform}-${process.arch}`. Always guard usage with `napi.isAvailable()`.
...
### `napi` (optional native mode via Rust `napi-rs`)

- `isAvailable(): boolean`
- `napi.x25519` with API parity to `wasm.x25519`
- `napi.ed25519` with API parity to `wasm.ed25519`
- `napi.axlsign` with API parity to `axlsign`
...
Rust crates reference: [../../rust/README.md](../../rust/README.md)
